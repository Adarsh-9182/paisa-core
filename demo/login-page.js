 ...
 *
 * Laid out the way an account page should be: the mark alone in the top-left
 * corner, and one narrow column in the middle of an otherwise empty page.
 * Nothing competes with the form, because on this page nothing else is the
 * job. The marketing argument lives at /, which is one click away.
 *
...
<style>
  :root {
    --bg:#FFFFFF; --wash:#F7F6F3; --line:#E4E0D8; --line-2:#EFEBE4;
    --ink:#17140F; --ink-2:#5F594F; --ink-3:#8E877C;
    --accent:#F26B1D; --accent-deep:#C24E08;
    --red:#B3372A; --red-soft:#FBEBE9; --red-line:#F0CBC5;
  }
  * { box-sizing:border-box; margin:0; }
  ...
  /* ---- the corner ------------------------------------------------------
     The mark sits in the top-left and nowhere else. It is the way back to
     the site, and on a page with one job it is the only chrome that earns
     its place.                                                            */
  header { display:flex; align-items:center; justify-content:space-between;
           padding:22px 24px; }
  .mark { display:inline-flex; align-items:center; gap:9px; text-decoration:none; color:var(--ink); }
  .mark .box { width:28px; height:28px; border-radius:8px; background:var(--accent); color:#fff;
               display:grid; place-items:center; font-weight:700; font-size:15px; }
  .mark b { font-weight:640; font-size:16px; letter-spacing:-.015em; }
  .top-link { color:var(--ink-2); text-decoration:none; font-size:13.5px; font-weight:520;
              padding:8px 14px; border-radius:999px; transition:background .15s, color .15s; }
  .top-link:hover { background:var(--wash); color:var(--ink); }

  /* ---- the column ---- */
  main { flex:1; display:flex; align-items:center; justify-content:center;
         padding:12px 22px 76px; }
  .col { width:100%; max-width:392px; }

  h1 { font-size:31px; font-weight:620; letter-spacing:-.03em; text-align:center; line-height:1.18; }
  .sub { color:var(--ink-2); font-size:14.5px; text-align:center; margin-top:12px; line-height:1.5; }

  form { display:flex; flex-direction:column; gap:12px; margin-top:28px; }
  label { display:block; font-size:12.5px; font-weight:560; color:var(--ink-2); margin-bottom:7px; }
  input { width:100%; height:52px; background:var(--bg); border:1px solid var(--line);
          color:var(--ink); border-radius:12px; padding:0 16px; font-size:15px;
          font-family:inherit; outline:none; transition:border-color .15s, box-shadow .15s; }
  input::placeholder { color:var(--ink-3); }
  input:focus { border-color:var(--ink); box-shadow:0 0 0 1px var(--ink); }
  .go { height:52px; margin-top:4px; background:var(--ink); color:#fff; border:none;
        border-radius:12px; font-size:15px; font-weight:580; font-family:inherit; cursor:pointer;
        transition:background .15s, opacity .15s; }
  .go:hover { background:#2E2920; }
  ...
  /* Stacked, full-width, one per row — the shape a provider list should be,
     so adding a second provider later changes nothing but the count. */
  .provider { position:relative; display:flex; align-items:center; justify-content:center; gap:11px;
              width:100%; height:52px; border-radius:12px;
              border:1px solid var(--line); background:var(--bg); color:var(--ink);
              font-size:15px; font-weight:540; font-family:inherit; cursor:pointer;
              text-decoration:none; transition:background .15s, border-color .15s; }
  .provider:hover { background:var(--wash); border-color:#D8D2C8; }
  ...
  /* The other half of the page, always offered, never hidden in a menu. */
  .swap { margin-top:24px; text-align:center; font-size:14px; color:var(--ink-2); }
  .swap a { color:var(--accent-deep); font-weight:560; text-decoration:none; }
  .swap a:hover { text-decoration:underline; text-underline-offset:2px; }

  .note { margin-top:22px; padding:14px 15px; background:var(--wash);
          border:1px solid var(--line-2); border-radius:12px;
          font-size:13.5px; color:var(--ink-2); line-height:1.55; }
  .note a { color:var(--accent-deep); text-decoration:none; font-weight:540; }
  ...
  .error { display:flex; gap:9px; background:var(--red-soft); border:1px solid var(--red-line);
           color:var(--red); border-radius:12px; padding:12px 14px; font-size:13.5px;
           line-height:1.45; margin-top:20px; }

  .fine { margin-top:30px; text-align:center; font-size:12.5px; color:var(--ink-3); line-height:1.6; }
  .fine a { color:var(--ink-2); text-decoration:underline; text-underline-offset:2px; }

  @media (max-width:520px) { h1 { font-size:27px; } main { padding-bottom:48px; } }
  @media (prefers-reduced-motion:reduce) { * { transition:none !important; } }
</style>
</head>
<body>
  <header>
    <a class="mark" href="/"><span class="box">₹</span><b>Paisa</b></a>
    <a class="top-link" href="/site/contact">Book a demo</a>
  </header>

  <main>
    <div class="col">
      <h1>${view === "signup" ? "Create your account" : "Welcome back"}</h1>
      <p class="sub">${
        view === "signup"
          ? "One ledger, closed continuously, with an AI CFO that shows its working."
      ...
      ${
        view === "signup" && !signup
          ? `<div class="note">Paisa is invite-only while we onboard the first teams, so accounts
               ...
               ${google ? "If you have already been invited, Google sign-in below will let you in." : ""}
          ...
          : `<form id="f" method="POST" action="${view === "signup" ? "/api/register" : "/api/login"}">
              ...
              <div>
                ...
                <input id="email" name="email" type="email" placeholder="you@company.com"
                       autocomplete="username" required ${view === "signup" || google ? "" : "autofocus"}>
              </div>
              <div>
      ...
      <p class="fine">By continuing you agree to Paisa's terms.<br>
        Curious first? <a href="/try">Explore the demo books</a>.</p>
    </div>
  </main>
